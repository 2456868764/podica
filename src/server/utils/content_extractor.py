...
class ContentExtractor:
    ...
    def validate_url(url: str) -> bool:
        ...
        Returns:
            A tuple of (is_valid, url) where:
            - is_valid: True if URL is accessible, False otherwise
            - url: The original URL or the redirected URL if status_code is 302
        """
        try:
            ...
            # print(f"URL HEAD response: {response} status code: {response.status_code}")
            
            # if response.status_code == 302:
            #     # 获取重定向的URL
            #     redirect_url = response.headers.get('Location', url)
            #     # 如果重定向URL是相对路径，需要构建完整URL
