...
# Only set HF_HUB_CACHE if not already set (allows override from parent module)
...
class IndexTTS2:
    def __init__(
        ...
        print(f"index tts infer_v2HF cache directory: {hf_cache_dir}")
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path,
        ...
        # Ensure emo_alpha is a float to avoid type comparison errors
        ...
        if output_path:
            ...
            # Use soundfile to save audio (more reliable, doesn't require torchcodec)
            try:
                import soundfile as sf
                # Convert tensor to numpy array
                wav_np = wav.type(torch.int16).numpy()
                # Handle mono/stereo
                if len(wav_np.shape) > 1:
                    wav_np = wav_np.T  # Transpose for soundfile format (channels x samples)
                else:
                    wav_np = wav_np.squeeze()
                sf.write(output_path, wav_np, sampling_rate)
                print(">> wav file saved to:", output_path)
            except ImportError:
                # Fallback to torchaudio if soundfile is not available
                try:
                    torchaudio.save(output_path, wav.type(torch.int16), sampling_rate)
                    print(">> wav file saved to:", output_path)
                except Exception as e:
                    # Last resort: use scipy.io.wavfile
                    try:
                        ...
                        if len(wav_np.shape) > 1:
                            ...
                            wav_np = wav_np.squeeze()
                        wavfile.write(output_path, sampling_rate, wav_np)
                        print(">> wav file saved to:", output_path)
                    except Exception as e2:
                        print(f">> Failed to save audio: {e}, fallback also failed: {e2}")
                        raise
            if stream_return:
                return None
            yield output_path
