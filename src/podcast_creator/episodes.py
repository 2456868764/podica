...
class EpisodeProfile(BaseModel):
    ...
    # language
    language: str = Field("English", description="Language for the episode")
    
    # dialect (optional, only applicable for Chinese)
    dialect: Optional[str] = Field(None, description="Chinese dialect (e.g., 'mandarin', 'cantonese', 'sichuanese', 'henanese', 'shanghainese'). Only used when language is Chinese.")
    ...
    def validate_dialect(cls, v):
        if v is not None:
            v = v.strip()
            valid_dialects = ["mandarin", "cantonese", "sichuanese", "henanese", "shanghainese"]
            if v and v.lower() not in valid_dialects:
                logger.warning(f"Unknown dialect '{v}', but allowing it. Valid dialects: {valid_dialects}")
        return v
