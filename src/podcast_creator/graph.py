...
from .emotions import load_emotions_config
from .speed import load_speed_config
...
async def create_podcast(
    ...
    dialect: Optional[str] = None,  # Chinese dialect (e.g., "cantonese", "sichuanese", "henanese", "shanghainese")
    ...
    if episode_profile:
        ...
        print(f"episode_config: {episode_config}")
        ...
        qa_provider = outline_provider
        qa_model = outline_model
        ...
        # Get dialect from episode config if not provided
        resolved_dialect = dialect or getattr(episode_config, 'dialect', None)
        ...
        if briefing_suffix:
            resolved_briefing = f"{resolved_briefing}\n\nAdditional focus: {briefing_suffix}"
        # language
        language = language or "中文"
        resolved_dialect = dialect  # Keep dialect as-is if provided
        language = "chinese" if language == "中文" else "english"
    ...
    print(f"resolved_briefing: {resolved_briefing}")
    # exit()
    ...
    initial_state = PodcastState(
        ...
        # language
        language=language,
        dialect=resolved_dialect,  # Add dialect to state
        speaker_profile=speaker_profile,
        emotions_config=emotions_config,
        speed_config=speed_config,
    )

    print(f"init state: {initial_state}")
