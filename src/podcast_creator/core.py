...
class Segment(BaseModel):
    ...
    size: Literal["short", "medium", "long"] = Field(
        default="short", description="Size of the segment"
    )
...
class Dialogue(BaseModel):
    ...
    emotion: str = Field(..., description="Emotion category or text description of the dialogue")
...
def create_validated_transcript_parser(valid_speaker_names: List[str], valid_emotion_names: List[str], valid_speed_names: List[str]):
    """
    ...
    class ValidatedDialogue(BaseModel):
        ...
        #     cleaned_name = v.strip()
        #     if cleaned_name not in valid_emotion_names:
        #         raise ValueError(
        #             f"Invalid emotion name '{cleaned_name}'. Must be one of: {', '.join(valid_emotion_names)}"
        #         )
        #     return cleaned_name
    ...
    return PydanticOutputParser[ValidatedTranscript](pydantic_object=ValidatedTranscript)
