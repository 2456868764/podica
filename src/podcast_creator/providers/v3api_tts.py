...
class V3APITextToSpeechModel(OpenAITextToSpeechModel):
    """V3 API Text-to-Speech provider implementation for custom endpoints.
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.base_url = (
            ...
            os.getenv("V3API_BASE_URL")
        )

        self.api_key = (
            api_key or
            config.get("api_key") or
            os.getenv("V3API_API_KEY")
        )

        # Validation
        if not self.base_url:
            raise ValueError(
                "V3 API base URL is required. "
                "Set V3_API_BASE_URL environment variable or provide base_url in config."
            )
    ...
    def _handle_error(self, response) -> None:
        ...
        if response.status_code >= 400:
            ...
            raise RuntimeError(f"V3 API TTS endpoint error: {error_message}")
    ...
    def available_voices(self) -> Dict[str, Voice]:
        ...
        try:
            ...
            for vid in ["alloy","echo", "fable", "onyx"]:
                voices[vid] = Voice(
                    name=vid,
                    id=vid,
                    gender="Male",
    ...
    def capability(self) -> TTSCapability:
        ...
        Returns:
            TTSCapability: Capability information for V3 API TTS
        """
        # Get default voices from available_voices
        default_voices = list(self.available_voices.keys())
        
        # V3 API TTS may support paralinguistic controls (depends on backend)
        voice_tags = []
        
        return TTSCapability(
            supported_languages=["zh", "en"],  # Chinese and English (assumed)
            supported_dialects=["mandarin"],
            supports_instructions=False,  # V3 API TTS doesn't support instructions parameter
            supports_custom_voice=False,  # V3 API TTS doesn't support custom voice upload
            default_voices=default_voices if default_voices else ["default"],
            supports_voice_tags=False,  # V3 API may support paralinguistic controls
            available_voice_tags=voice_tags,
            metadata={
                "description": "V3 API TTS provider (OpenAI-compatible endpoint)",
