...
class LaoZhangTextToSpeechModel(OpenAITextToSpeechModel):
    """LaoZhang Text-to-Speech provider implementation for custom endpoints.

    This provider extends OpenAI's TTS implementation to work with any V3 API-compatible
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not self.base_url:
            raise ValueError(
                "LaoZhang base URL is required. "
                "Set LAOZHANG_BASE_URL environment variable or provide base_url in config."
            )

        # Use a default API key if none is provided (some endpoints don't require authentication)
        if not self.api_key:
            logger.warning("No API key provided for V3 API endpoint. Using default 'not-required' value.")
    ...
    def _handle_error(self, response) -> None:
        ...
        if response.status_code >= 400:
            ...
            logger.debug(f"V3 API endpoint error: {response.text}")
            ...
            raise RuntimeError(f"LaoZhang API TTS endpoint error: {error_message}")
    ...
    def available_voices(self) -> Dict[str, Voice]:
        ...
        try:
            ...
            for vid in ["nova", "sage", "shimmer", "verse"]:
                voices[vid] = Voice(
                    name=vid,
                    id=vid,
                    gender="Female",
                    language_code="auto",
                    description=f"OpenAI TTS voice {vid}",
                ) 

            for vid in ["alloy","echo", "ash", "coral", "fable", "onyx"]:
                voices[vid] = Voice(
                    name=vid,
                    id=vid,
                    gender="Male",
    ...
    def capability(self) -> TTSCapability:
        ...
        Returns:
            TTSCapability: Capability information for LaoZhang TTS
        """
        # Get default voices from available_voices
        default_voices = list(self.available_voices.keys())
        
        # LaoZhang TTS may support paralinguistic controls (depends on backend)
        voice_tags = []

        return TTSCapability(
            supported_languages=["zh", "en"],  # Chinese and English (assumed)
            supported_dialects=["mandarin"],
            supports_instructions=False,  # LaoZhang TTS doesn't support instructions parameter
            supports_custom_voice=False,  # LaoZhang TTS doesn't support custom voice upload
            default_voices=default_voices if default_voices else ["default"],
            supports_voice_tags=False,  # LaoZhang may support paralinguistic controls
            available_voice_tags=voice_tags,
            metadata={
                "description": "LaoZhang TTS provider (OpenAI-compatible endpoint)",
                "voice_selection": "Select from predefined voice list",
                "voice_tags_format": "Paralinguistic controls: laughter, sighs, breaths, pauses. May be supported depending on backend."
            }
        )
    ...
    def generate_speech(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"Generating speech with payload: {payload}")
