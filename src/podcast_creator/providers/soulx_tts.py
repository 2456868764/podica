...
class SoulXTextToSpeechModel(OpenAITextToSpeechModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.base_url = (
            ...
            os.getenv("SOULX_BASE_URL")
        )

        self.api_key = (
            api_key or
            config.get("api_key") or
            os.getenv("SOULX_API_KEY")
        )
        ...
        if not self.base_url:
            raise ValueError(
                ...
                "Set SOULX_BASE_URL environment variable or provide base_url in config."
            )
    ...
    def available_voices(self) -> Dict[str, Voice]:
        """Get available voices for IndexTTS.

        Soulxuses reference audio files for voice cloning.
        This returns the default voice mappings from config.

        Returns:
            Dict[str, Voice]: Dictionary of available voices with their information
        """
        voices: Dict[str, Voice] = {}
        # Soulxdefault voices from config
        voice_list = [
            "default", "郭德纲", "蜡笔小新", "男声1", "男声2",
            "女生_安陵容", "女生_明兰", "女生_新闻联播", "女生_甄嬛",
            "佩奇", "童声_男", "童声_女", "星爷","男生_soulx1", "女生_soulx1"
        ]
        
        for vid in voice_list:
            # Determine gender based on voice name
            if "女" in vid or vid in ["佩奇", "童声_女"]:
                gender = "Female"
            elif "男" in vid or vid in ["童声_男"]:
                gender = "Male"
            else:
                gender = "Unknown"
    ...
    def capability(self) -> TTSCapability:
        ...
        Returns:
            TTSCapability: Capability information for IndexTTS
        """
        # Get default voices from available_voices
        default_voices = list(self.available_voices.keys())
        
        # Soulxsupports paralinguistic controls (voice tags)
        # These can be embedded in text or controlled via instructions
        voice_tags = [
            "[laughter]",           # 笑声
            "[sigh]",               # 叹息
            "[breathing]",          # 呼吸声
        ...
        return TTSCapability(
            ...
            supported_dialects=["mandarin", "cantonese", "sichuanese", "henanese"],  # Chinese dialects supported by SoulX
            supports_instructions=False,  # Soulxsupports emo_text/instructions
            supports_custom_voice=True,  # Soulxsupports custom reference audio
            default_voices=default_voices if default_voices else ["default"],
            supports_voice_tags=True,  # Soulxsupports paralinguistic controls
            available_voice_tags=voice_tags,
            metadata={
                "description": "Soulx supports zero-shot voice cloning with reference audio",
                "instruction_format": "Text-based emotion/style instructions (emo_text)",
                "custom_voice_format": "WAV audio file as reference_audio parameter",
                "voice_tags_format": "Paralinguistic controls: laughter, sighs, breaths, etc. Can be embedded in text or controlled via instructions"
            }
        )
    ...
    def generate_speech(
        ...
        **kwargs
        ...
        try:
            ...
            # print(f"Request soulx payload: {payload}")
            # Generate speech
...
if __name__ == "__main__":
    ...
    async def test_index_tts():
        """测试 SOULXTTSTextToSpeechModel 类的功能"""
        print("开始测试 SOULXTTSTextToSpeechModel...")
        
        # 设置环境变量（如果需要）
        os.environ.setdefault("SOULX_TTS_BASE_URL", "http://localhost:9000/v1")  # 替换为实际的 API 地址
        
        # 创建 TTS 模型实例
        try:
            tts_model = SoulXTextToSpeechModel(
                model_name="soulx",
                base_url=os.environ.get("SOULX_TTS_BASE_URL")
            )
            ...
            # 测试生成语音（同步方法）
            test_text = "这是一个测试文本，用于验证 Soulx模型的功能。"
            voice_id = "男声1"  # 使用一个可用的声音 ID
            output_file = "index_tts_test.wav"
            ...
            print("\n测试异步语音生成...")
            try:
                async_output_file = "index_tts_async_test.wav"
