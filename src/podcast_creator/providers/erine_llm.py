...
class ErineLanguageModel(OpenAILanguageModel):
    """Baidu Erine language model implementation for custom endpoints."""

    base_url: Optional[str] = None
    api_key: Optional[str] = None

    def __post_init__(self):
        """Initialize Erine configuration."""
        ...
        self.base_url = (
            ...
            os.getenv("ERNIE_BASE_URL", "https://qianfan.baidubce.com/v2")
        )

        self.api_key = (
            self.api_key or 
            self._config.get("api_key") or 
            os.getenv("ERNIE_API_KEY")
        )
        ...
        if not self.api_key:
           raise ValueError(
                "Erine API key is required. "
                "Set ERNIE_API_KEY environment variable or provide api_key in config."
            )
    ...
    def _handle_error(self, response) -> None:
        ...
        if response.status_code >= 400:
            ...
            logger.debug(f"Erine endpoint error: {response.text}")
            ...
            raise RuntimeError(f"Erine endpoint error: {error_message}")
    
    def _normalize_response(self, response_data: Dict[str, Any]) -> "ChatCompletion":
        """Normalize Erine response to our format with graceful fallback."""
    ...
    def _normalize_chunk(self, chunk_data: Dict[str, Any]) -> "ChatCompletionChunk":
        """Normalize Erine stream chunk to our format with graceful fallback."""
    ...
    def _get_api_kwargs(self, exclude_stream: bool = False) -> Dict[str, Any]:
        ...
        # For Erine endpoints, we attempt all features
    ...
    def models(self) -> List[Model]:
        ...
        try:
            ...
            logger.debug(f"Could not fetch models from Erine endpoint: {e}")
            return []

    def _get_default_model(self) -> str:
        """Get the default model name.
        
        For Erine endpoints, we use a generic default
...
if __name__ == "__main__":
    ...
    DEFAULT_BASE_URL = os.getenv("ERINE_BASE_URL", "https://ERINE.aliyuncs.com/compatible-mode/v1")
    DEFAULT_API_KEY = os.getenv("ERINE_API_KEY", "sk-xxxx")
    ...
    def test_validation_error_without_base_url() -> bool:
        ...
        try:
            ...
            ErineLanguageModel(config={"api_key": DEFAULT_API_KEY})
    ...
    def test_models_endpoint(base_url: str, api_key: str) -> bool:
        ...
        try:
            llm = ErineLanguageModel(config={"base_url": base_url, "api_key": api_key})
            models = llm.models
    ...
    print("=== ErineLanguageModel 内嵌测试开始 ===")
    print(f"使用 base_url: {DEFAULT_BASE_URL}")
    print("提示：如需修改，请设置环境变量 ERINE_BASE_URL / ERINE_API_KEY")
