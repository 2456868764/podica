...
class IndexTTSTextToSpeechModel(OpenAITextToSpeechModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.base_url = (
            ...
            os.getenv("INDEXTTS_BASE_URL")
        )

        self.api_key = (
            api_key or
            config.get("api_key") or
            os.getenv("INDEXTTS_API_KEY")
        )
        ...
        if not self.base_url:
            raise ValueError(
                ...
                "Set INDEXTTS_BASE_URL environment variable or provide base_url in config."
            )
    ...
    def available_voices(self) -> Dict[str, Voice]:
        ...
        # IndexTTS default voices from config
        voice_list = [
        ...
        for vid in voice_list:
            ...
            voices[vid] = Voice(
                ...
                gender=gender,
                language_code="auto",
                description=f"IndexTTS voice {vid}",
            )
        return voices
    ...
    def capability(self) -> TTSCapability:
        ...
        # IndexTTS supports paralinguistic controls (voice tags)
        ...
        return TTSCapability(
            ...
            supported_dialects=["mandarin", "cantonese", "sichuanese", "henanese"],  # Chinese dialects supported by IndexTTS
            supports_instructions=True,  # IndexTTS supports emo_text/instructions
            supports_custom_voice=True,  # IndexTTS supports custom reference audio
            default_voices=default_voices if default_voices else ["default"],
            supports_voice_tags=False,  # IndexTTS supports paralinguistic controls
            available_voice_tags=voice_tags,
            metadata={
                "description": "IndexTTS supports zero-shot voice cloning with reference audio",
    ...
    def generate_speech(
        ...
        **kwargs
        ...
        try:
            ...
            # print(f"Request IndexTTS payload: {payload}")
            # Generate speech
...
if __name__ == "__main__":
    ...
    async def test_index_tts():
        """测试 IndexTTSTextToSpeechModel 类的功能"""
        print("开始测试 IndexTTSTextToSpeechModel...")
        
        # 设置环境变量（如果需要）
        os.environ.setdefault("INDEX_TTS_BASE_URL", "http://localhost:9000/v1")  # 替换为实际的 API 地址
        
        # 创建 TTS 模型实例
        try:
            tts_model = IndexTTSTextToSpeechModel(
                model_name="index-tts",
                base_url=os.environ.get("INDEX_TTS_BASE_URL")
            )
            ...
            # 测试生成语音（同步方法）
            test_text = "这是一个测试文本，用于验证 IndexTTS 模型的功能。"
