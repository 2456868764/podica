...
class PodcastConfig(BaseModel):
    ...
    emotions_config: Optional[Union[str, Dict[str, Any]]] = Field(
        None, description="Path to emotions config JSON or inline config dict"
    )
    ...
    @field_validator("emotions_config")
    @classmethod
    def validate_emotions_config(
        cls, v: Optional[Union[str, Dict[str, Any]]]
    ) -> Optional[Union[str, Dict[str, Any]]]:
        ...
        if isinstance(v, str):
            ...
            path = Path(v)
            if not path.exists():
                raise ValueError(f"Emotions config file not found: {v}")
            if not path.suffix == ".json":
                raise ValueError(f"Emotions config must be a JSON file: {v}")
        elif isinstance(v, dict):
            # Validate dictionary structure
            try:
                ...
                raise ValueError(f"Invalid emotions config structure: {e}")
    ...
    def configure(
        ...
        for key, val in updates.items():
            if key == "templates" and isinstance(val, dict):
                ...
                EpisodeConfig(**val)
            elif key == "emotions_config" and isinstance(val, dict):
                # Validate emotions config structure
                from .emotions import EmotionConfig
                EmotionConfig(**val)
